...
Once a topic is secured, the investigative phase employs the ``Eightfold Path'' to rigorously define the problem and select evaluation criteria before data collection begins \citep{bardach_practical_2024}. The literature search must be executed systematically; researchers apply the Boolean logic and database limits detailed in \Cref{tab:SearchTerms} to construct reproducible search strings \citep{watson_how_2020}. This adherence to the PRISMA reporting guidelines ensures that evidence synthesis is comprehensive and free from selection bias, distinguishing rigorous policy analysis from partisan advocacy \citep{page_prisma_2021}.
